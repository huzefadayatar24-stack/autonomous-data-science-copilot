...
st.sidebar.success("Powered by Groq LPUs for fast execution.")
...
def call_llm(prompt: str, temperature: float = 0.2) -> str:
    """Helper to query Groq with explicit error surfacing."""
    if not client:
        st.error("AI Client not initialized. Please provide a valid Groq API Key.")
        st.stop()
...
def extract_python_code(content: str) -> str:
    """Extracts raw executable code safely from markdown blocks."""
    fence = chr(96) * 3
    if f"{fence}python" in content:
        return content.split(f"{fence}python")[1].split(fence)[0].strip()
    elif fence in content:
        return content.split(fence)[1].split(fence)[0].strip()
...
if df is not None:
    ...
    with col2:
        ...
        if target_col is not None:
            st.metric("Target Variable", str(target_col))
...
3. Output ONLY Python code inside {fence}python ... {fence} blocks.
"""
                    ...
                    try:
                        ...
                        st.warning(f"Dynamic plotting fallback due to error: {e}")
        ...
        if st.button("Run Data Cleaning Agent"):
            if not client:
                ...
                with st.spinner("Writing transformation logic..."):
                    clean_prompt = f"Data sample:\n{df.head(5).to_dict()}\nWrite `clean_dataset(df)` to handle missing values, encode categories, and drop zero-variance columns. Return ONLY executable python code inside {fence}python ... {fence} blocks."
    ...
    with tabs[2]:
        ...
        if st.button("Start Optimization Pipeline", type="primary"):
            if not client:
                ...
                st.error("Please select a target column in the sidebar.")
            else:
                ...
                for r in range(1, rounds + 1):
                    ...
                    arch_prompt = f"Target: '{target_col}'. History:\n{history_txt}\nWrite `def train_and_evaluate(df):` returning `(float(score), trained_model)`. Output ONLY executable code inside {fence}python ... {fence} blocks."
